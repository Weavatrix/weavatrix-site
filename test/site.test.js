...
test('published product versions, MIT licenses, and native benchmark stay current', () => {
    ...
    for (const release of ['Core <small>1.1.1', 'Refactor <small>0.1.5', 'Online <small>0.3.1']) {
        assert.ok(index.includes(release), `${release} is shown on the product grid`)
    }
    assert.match(index, /weavatrix-rust <small>2\.0\.1/)
    ...
    assert.match(index, /187 files became 1,516 nodes and 7,223 typed edges/)
    assert.match(index, /168\.35 ms/)
    assert.match(index, /11\.620 ms/)
    assert.match(index, /7\.542 ms/)
    assert.match(index, /1\.134 ms/)
    assert.match(index, /1,000 hot calls/)
    assert.match(index, /500\.780 ms/)
    assert.match(index, /1\.230 ms/)
    assert.match(index, /55\.490 ms/)
    assert.match(index, /146\.70 calls\/s/)
    assert.match(index, /p50 7\.040 \/ p95 8\.750 \/ p99 9\.460 \/ max 67\.790 ms/)
    assert.match(index, /87\.69% lines \/ 80\.47% functions \/ 85\.27% regions/)
...
test('release metadata and every local page reference resolve', () => {
    ...
    assert.equal(metadata.softwareVersion, '1.1.1')
